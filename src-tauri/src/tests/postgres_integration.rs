...
async fn start_pg() -> (DbConnection, testcontainers::ContainerAsync<Postgres>) {
    ...
    let pool = sqlx::postgres::PgPool::connect_with(opts)
        .await
        .expect("failed to connect to postgres");

    (DbConnection::Postgres(pool), container)
}
...
async fn pg_list_databases() {
    let (conn, _c) = start_pg().await;
    let dbs = conn.list_databases().await.expect("list_databases failed");
    assert!(!dbs.is_empty());
    assert!(dbs.iter().any(|d| d == "postgres"));
}

// ── list_tables_for_db ────────────────────────────────────────────────────────

#[tokio::test]
async fn pg_list_tables_for_db() {
    let (conn, _c) = start_pg().await;
    conn.execute_query(
        "CREATE TABLE _list_tables_test (id SERIAL PRIMARY KEY)",
    )
    .await
    .unwrap();

    let tables = conn.list_tables_for_db("postgres").await.unwrap();
...
#[tokio::test]
async fn pg_get_table_columns() {
    let (conn, _c) = start_pg().await;
    conn.execute_query(
        "CREATE TABLE _col_test (id SERIAL PRIMARY KEY, name TEXT NOT NULL, score FLOAT)",
    )
    .await
    .unwrap();
...
#[tokio::test]
async fn pg_get_column_keys() {
    let (conn, _c) = start_pg().await;
    conn.execute_query(
        "CREATE TABLE _keys_parent (id SERIAL PRIMARY KEY)",
    )
    .await
    .unwrap();
    conn.execute_query(
        "CREATE TABLE _keys_child (
            ...
            parent_id INT REFERENCES _keys_parent(id)
        )",
    )
    .await
    .unwrap();
...
async fn pg_get_schema() {
    let (conn, _c) = start_pg().await;
    conn.execute_query("CREATE TABLE _schema_test (id SERIAL, name TEXT)")
        .await
        .unwrap();

    let schema = conn.get_schema("postgres").await.unwrap();
...
#[tokio::test]
async fn pg_run_select_query() {
    let (conn, _c) = start_pg().await;
    let result = conn
        .execute_query("SELECT 1::int AS num, 'hello'::text AS greeting, true::bool AS flag")
        .await
        .unwrap();

    assert_eq!(result.columns, vec!["num", "greeting", "flag"]);
    assert_eq!(result.row_count, 1);
}

#[tokio::test]
async fn pg_run_dml_query() {
    let (conn, _c) = start_pg().await;
    conn.execute_query("CREATE TABLE _dml_test (id SERIAL PRIMARY KEY, val TEXT)")
        .await
        .unwrap();
...
#[tokio::test]
async fn pg_get_table_data_with_filter() {
    let (conn, _c) = start_pg().await;
    conn.execute_query(
        "CREATE TABLE _data_test (id SERIAL PRIMARY KEY, name TEXT)",
    )
    .await
    .unwrap();
    conn.execute_query(
...
#[tokio::test]
async fn pg_count_rows() {
    let (conn, _c) = start_pg().await;
    conn.execute_query(
        "CREATE TABLE _count_test (id SERIAL PRIMARY KEY, name TEXT)",
    )
    .await
    .unwrap();
    conn.execute_query(
...
#[tokio::test]
async fn pg_insert_and_delete_rows() {
    let (conn, _c) = start_pg().await;
    conn.execute_query(
        "CREATE TABLE _mut_test (id SERIAL PRIMARY KEY, val TEXT)",
    )
    .await
    .unwrap();
...
#[tokio::test]
async fn pg_submit_changes() {
    use crate::models::RowUpdate;

    let (conn, _c) = start_pg().await;
    conn.execute_query(
        "CREATE TABLE _submit_test (id SERIAL PRIMARY KEY, val TEXT)",
    )
    .await
    .unwrap();
    conn.execute_query(
        "INSERT INTO _submit_test (val) VALUES ('initial')",
    )
    .await
    .unwrap();
    ...
    // Update the row and insert a new one
