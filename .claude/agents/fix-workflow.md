...
model: sonnet
tools: Bash, Read, Edit, Glob, Grep, mcp__github__list_workflow_runs, mcp__github__get_workflow_run, mcp__github__get_workflow_run_logs, mcp__github__list_workflows
---
...
Determine the repo owner and name:

```bash
gh repo view --json nameWithOwner --jq .nameWithOwner
```

Call `mcp__github__list_workflow_runs` with `status: "failure"` and `per_page: 5`.
...
- JS/TS: `npm run build` or `npm run typecheck` or `npm test -- --run`
...
git commit -m "<conventional commit message describing what was fixed>"
git push
```

Commit message format: `fix: <concise description>` (e.g. `fix: resolve type error in storage.rs`)
