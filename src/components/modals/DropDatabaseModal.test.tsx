...
import DropDatabaseModal from './DropDatabaseModal';
...
function renderModal(props = {}) {
  return render(<DropDatabaseModal {...defaultProps} {...props} />);
}
...
describe('DropDatabaseModal', () => {
  it('renders title', () => {
    renderModal();
    expect(screen.getByRole('heading', { name: 'Drop Database' })).toBeInTheDocument();
  });

  it('shows database name in warning and label', () => {
    renderModal();
    expect(screen.getAllByText('mydb').length).toBeGreaterThan(0);
  });

  it('Drop Database button is disabled when confirmation is empty', () => {
    renderModal();
    expect(screen.getByRole('button', { name: 'Drop Database' })).toBeDisabled();
  });

  it('Drop Database button is disabled when confirmation does not match', () => {
    renderModal();
    fireEvent.change(screen.getByRole('textbox'), { target: { value: 'wrong' } });
    expect(screen.getByRole('button', { name: 'Drop Database' })).toBeDisabled();
  });

  it('Drop Database button is enabled when confirmation matches database name', () => {
    renderModal();
    fireEvent.change(screen.getByRole('textbox'), { target: { value: 'mydb' } });
  ...
  it('calls disconnect_database and execute_ddl with postgres SQL on drop', async () => {
